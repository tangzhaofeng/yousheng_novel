...
class pagerInit { 
	...
	.InitPHP_pages {font:12px/1.6em Helvetica, Arial, sans-serif;overflow:hidden; text-align:center; font-family:Verdana;margin-bottom:5px;padding:12px 0px;}
	...
	private function get_first_last_html($page_num, $url) {
		...
		$first = "<a href='{$url}1' class='first'>首页</a>";
		$last  = "<a href='{$url}{$page_num}' class='last'>尾页</a>";
	...
	private function get_pager_next_back_html($url, $page, $page_num) {
		...
		$next_page = $page + 1;
		$next = "<a href='{$url}{$next_page}' class='next'>下一条</a>";
		if ($page == $page_num) $next = '';
		$back_page = $page - 1;
		$back = "<a href='{$url}{$back_page}' class='back'>上一条</a>";
		if ($page == 1) $back = '';
	...
	private function get_select_html($page_num, $url, $page) {
		...
		for ($i=1; $i<=$page_num; $i++){
            ...
            $html.="<option value='{$url}{$i}' {$selected}>{$i}</option>";
        }
		$html .= '</select>';
		return $html;
	}
