...
class dbInit extends sqlbuildInit {   
	 ...
	 */
	public function get_all($table_name, $num = 20, $offest = 0, $field = array(),$other=false,$id_key = 'id', $sort = 'DESC') {
		$where = $this->build_where($field,$other);
		...
		$count = $this->get_count($table_name, $field,$other);
		$this->set_default_link_id(); //设置默认的link_id
	 ...
	 * @return int
	 */
	public function get_count($table_name, $field = array(),$other=false) {
		$where = $this->build_where($field,$other);
